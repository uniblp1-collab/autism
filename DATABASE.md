...
| color | string | hex |
| priority | int | Влияет на сортировку внутри раздела (`PrismaCardRepository.search`: `orderBy priority desc, title asc`). Родитель может "поставить карточку на первое место" из режима редактирования на экране ребёнка — см. §3 (`PATCH /cards/:id/promote`). |
...
- **Загрузка/замена картинки карточки** — `POST /cards/:id/image` (multipart, поле `file`, любой авторизованный родитель — не только `ADMIN`, см. `cards.controller.ts`). Файл валидируется на бэкенде (не только на фронтенде): mime-type строго `image/jpeg|png|webp`, размер ≤ 5 МБ (`apps/backend/src/storage/card-image-validator.ts`). Сохраняется на диск backend-контейнера через `StorageService` (`apps/backend/src/storage`) — масштаб проекта (один инстанс backend, без горизонтального масштабирования) не оправдывает S3-совместимое хранилище; результирующий относительный путь (`/uploads/cards/<файл>`) пишется в `cards.imageUrl` тем же `CardRepository.update`, которым пользуется обычный `UpdateCardUseCase`. Замена картинки и удаление карточки удаляют старый файл с диска (`StorageService.deleteCardImage`), чтобы не копить файлы-сироты.
  ...
  - **Базовый комплект собственных иллюстраций** — `packages/database/seed-assets/cards/` (29 файлов) + соответствие "название карточки → файл" в `CARD_IMAGE_FILES` (`packages/database/prisma/seed-data.ts`). Применяется автоматически прямо внутри `prisma/seed.ts` (функция `resolveSeedCardImageUrl`) при обычном `prisma db seed` — **не отдельным ручным скриптом**: seed выполняется в том же окружении/контейнере, что и backend (см. `packages/docker/dev-up.sh`, где `prisma db seed` гоняется на каждом запуске), поэтому пережимает исходник через `sharp` (те же параметры, что и `StorageService`: ≤640px, WebP q82) и пишет файл напрямую в `UPLOAD_DIR` — тот же диск, с которого backend отдаёт `/uploads/cards/*`. Имя выходного файла детерминированное (`seed-<ascii-имя-ассета>.webp`, не `randomUUID`), поэтому повторный seed не плодит файлы и не требует сети/токенов/запущенного backend по HTTP. Не хардкод: сами файлы — просто исходные ассеты, `imageUrl` в БД — обычное поле, простановленное тем же seed-скриптом, что заводит остальные поля библиотечных карточек, ничего не зашито в код компонентов фронтенда.
    - **Важно про UPLOAD_DIR:** `resolveUploadDir()` в `seed.ts` обязан резолвить ту же директорию, что и `StorageService` (`apps/backend/src/storage/storage.service.ts`) — если `UPLOAD_DIR` нигде не задан, backend фактически использует хардкод-дефолт `/app/uploads/cards`, а не относительный путь из `.env.example`; seed.ts повторяет ровно этот дефолт, иначе картинки уйдут не туда, откуда backend их отдаёт (это и была причина, по которой базовые картинки не появлялись на карточках при первой версии фикса — отдельный ручной скрипт, требовавший явного запуска, который никто не запускал после обычного деплоя/`dev-up.sh`).
  - **"Поставить карточку на первое место в разделе"** (запрос заказчика: популярную карточку — в начало) — `PATCH /cards/:id/promote`, тело `{ childId }`. `PromoteCardUseCase` берёт все карточки того же раздела и типа, которые реально показываются этому ребёнку вместе (библиотечные + его кастомные, тот же набор, что и `useCards({ includeCustom: true })` на экране ребёнка), и одним `$transaction` переприсваивает `priority`: целевая карточка получает наибольшее значение, остальные — сохраняют взаимный порядок со сдвигом. `childId` обязателен и проверяется через `ChildAccessService` (иначе можно было бы переставить карточки чужого ребёнка). Кнопка — иконка "в начало" в левом нижнем углу карточки, видна только в режиме редактирования (`packages/ui/src/components/CardButton.tsx`, проп `onPromote`).
