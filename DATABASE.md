...
| imageUrl | string, nullable | путь в MinIO/S3; `NULL` до первой загрузки картинки через `POST /admin/cards/:id/image` (см. §5) — на карточке рендерится иконка-заглушка вместо изображения |
| color | string | hex |
...
- **Загрузка/замена картинки карточки** — `POST /admin/cards/:id/image` (multipart, поле `file`, только `ADMIN`). Файл валидируется на бэкенде (не только на фронтенде): mime-type строго `image/jpeg|png|webp`, размер ≤ 5 МБ (`apps/backend/src/storage/card-image-validator.ts`). Загружается в S3-совместимое хранилище (MinIO в dev/test) через `StorageService` (`apps/backend/src/storage`), результирующий URL пишется в `cards.imageUrl` тем же `CardRepository.update`, которым пользуется обычный `UpdateCardUseCase`.
  - Бакет создаётся автоматически при старте (`StorageService.onModuleInit`) с публичной политикой на чтение — **TODO(безопасность, MVP-упрощение):** для продакшена нужен приватный бакет + presigned GET URL или CDN, публичный бакет — сознательное временное решение, не финальный дизайн.
  - `S3_ENDPOINT` (внутренний, для бэкенда) и `S3_PUBLIC_URL` (внешний, для `<img>` в браузере) — разные переменные окружения, потому что в docker-compose это разные хосты (`minio:9000` внутри сети контейнеров vs `localhost:9000` снаружи).
