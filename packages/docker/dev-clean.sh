...
echo "а также базу данных Postgres и файлы MinIO (volumes)."
...
read -p "Удалить также скачанные образы (node, postgres, minio)? [y/N] " confirm_images
if [[ "$confirm_images" == "y" || "$confirm_images" == "Y" ]]; then
  echo "==> Удаляю образы проекта..."
  docker rmi -f \
    "$(docker images -q postgres:16)" \
    "$(docker images -q minio/minio)" \
    2>/dev/null || true
